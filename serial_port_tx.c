...
int main(int argc, char** argv)
{
    ...
    do {
        ...
        while( remain > 0 )
        {
            int     len = (remain < CONFIG_SLICE_SIZE) ? remain : CONFIG_SLICE_SIZE;

            printf("send %d (%lu / %u)\n", len, pCur - pBuf, filesize);

            _uart_send_bytes(hComm, pCur, len);

            pCur   += len;
            remain -= len;
        }

        memset(pBuf, 0xCC, filesize);
        _uart_send_bytes(hComm, pBuf, 16);
        _uart_send_bytes(hComm, pBuf, 8);
        printf("send SOE~~~~\n");
