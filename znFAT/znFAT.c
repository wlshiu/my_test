...
uint8_t tmpBuf[ZNFAT_BUF_SIZE];
uint8_t *znFAT_Buffer = tmpBuf; //znFAT的内部缓冲区, 使用者不可私自使用
...
uint32_t scccb_buf[CCCB_LEN]; //CCCB的缓冲区, 以连续簇段的方式来记录簇链
uint8_t  scccb_counter = 0;
uint32_t scccb_curval = 0;

uint8_t  scccb_curdev = (uint8_t)(-1);
#endif
#endif
...
uint32_t *pcccb_buf = scccb_buf;
uint32_t *pcccb_curval = &scccb_curval;
uint8_t  *pcccb_counter = &scccb_counter;

uint8_t  *pcccb_curdev = &scccb_curdev;
struct FileInfo *pcccb_cur_oc = (struct FileInfo *)0;
struct znFAT_Init_Args *pcccb_cur_initargs = (struct znFAT_Init_Args *)0;
#else
uint32_t *pcccb_buf = (uint32_t *)0;
uint32_t *pcccb_curval = (uint32_t *)0;
uint8_t  *pcccb_counter = (uint8_t *)0;
#endif

uint8_t get_next_cluster_in_cccb = 0; //用以标志是否在CCCB中查找下一簇
#endif
//----------------------------------------------------------
...
uint8_t  sexb_buf[ZNFAT_BUF_SIZE];
uint8_t  sexb_cur_dev = (uint8_t)(-1);
uint32_t sexb_cur_sec = 0;
...
#ifndef USE_ALONE_EXB
uint8_t *pexb_buf = sexb_buf;
#else
uint8_t *pexb_buf = (uint8_t *)0;
#endif
...
uint8_t Memory_Set(uint8_t *pmem, uint32_t len, uint8_t value)
{
    uint32_t i = 0;
...
uint8_t Memory_Compare(uint8_t *psmem, uint8_t *pdmem, uint32_t len)
{
    uint32_t i = 0;
...
uint8_t *Memory_Copy(uint8_t *pdmem, uint8_t *psmem, uint32_t len)
{
    uint32_t i = 0;
...
int8_t* StringCopy(int8_t *dest_str, int8_t *src_str)
{
    uint8_t i = 0;
...
uint32_t StringLen(int8_t *pstr)
{
    uint32_t len = 0;
...
uint32_t WStringLen(uint16_t *str)
{
    uint32_t i = 0;
...
uint8_t PGM_BYTE_FUN(uint8_t *ptr)
{
    return *(ptr);
}

uint16_t PGM_WORD_FUN(uint16_t *ptr)
{
    return *(ptr);
}

uint32_t PGM_DWORD_FUN(uint32_t *ptr)
{
    return *(ptr);
}

uint8_t *PGM2RAM(uint8_t *pdmem, uint8_t *psmem, uint32_t len)
{
    uint32_t i = 0;
...
*****************************************************************************/
uint8_t znFAT_Select_Device(uint8_t devno, struct znFAT_Init_Args *pinitargs) //选择设备
{
...
***********************************************************************************/
uint32_t Bytes2Value(uint8_t *dat, uint8_t len)
{
    uint32_t temp = 0;

    if(len >= 1) temp |= ((uint32_t)(dat[0]))    ;
    if(len >= 2) temp |= ((uint32_t)(dat[1])) << 8 ;
    if(len >= 3) temp |= ((uint32_t)(dat[2])) << 16;
    if(len >= 4) temp |= ((uint32_t)(dat[3])) << 24;
...
***********************************************************************************/
uint8_t Find_FSINFO_Sec(uint32_t *fsinfosec) //寻找FSINFO扇区, 里面有剩余簇数与可用的空簇
{
    uint32_t iSec = 0;
    struct FSInfo *pfsinfo;

    uint8_t head[4] = {'R', 'R', 'a', 'A'};
    uint8_t sign[4] = {'r', 'r', 'A', 'a'}; //FSINFO扇区的标志
...
/***********************************************************************************
 ...
 详解：从FAT表的最开始位置开始顺次查找, 直到第一个可用的空簇出现, 此空簇值将会被记录
       ...
       使用空簇时提供可用空簇的参考值.
***********************************************************************************/
uint8_t Search_Free_Cluster_From_Start(uint32_t *nFreeCluster)
{
    uint32_t iSec = 0;
    uint8_t  iItem = 0;
    ...
    for(iSec = 0; iSec < pInit_Args->FATsectors; iSec++)
    {
        ...
        for(iItem = 0; iItem < NITEMSINFATSEC; iItem++) //遍历所有簇项, 寻找空闲簇
        {
            ...
            {
                *nFreeCluster = ((iSec * NITEMSINFATSEC) + (uint32_t)iItem);
                return ERR_SUCC;
            }
        }
    }
...
***********************************************************************************/
#ifdef UPDATE_FSINFO
uint8_t Update_FSINFO(void) //更新FSINFO扇区
{
    struct FSInfo *pfsinfo;
    ...
    //写入剩余空簇数
    pfsinfo->Free_Cluster[0] = (uint8_t)( pInit_Args->Free_nCluster & 0X000000FF)    ;
    pfsinfo->Free_Cluster[1] = (uint8_t)((pInit_Args->Free_nCluster & 0X0000FF00) >> 8);
    pfsinfo->Free_Cluster[2] = (uint8_t)((pInit_Args->Free_nCluster & 0X00FF0000) >> 16);
    pfsinfo->Free_Cluster[3] = (uint8_t)((pInit_Args->Free_nCluster & 0XFF000000) >> 24);

    //写入下一空闲簇参考值, 并无多大意义
    pfsinfo->Next_Free_Cluster[0] = (uint8_t)( pInit_Args->Next_Free_Cluster & 0X000000FF)    ;
    pfsinfo->Next_Free_Cluster[1] = (uint8_t)((pInit_Args->Next_Free_Cluster & 0X0000FF00) >> 8);
    pfsinfo->Next_Free_Cluster[2] = (uint8_t)((pInit_Args->Next_Free_Cluster & 0X00FF0000) >> 16);
    pfsinfo->Next_Free_Cluster[3] = (uint8_t)((pInit_Args->Next_Free_Cluster & 0XFF000000) >> 24);
...
/***********************************************************************************
 ...
 详解：文件系统初始化函数, 将完成文件系统初始化参数集合的装入, 为以后的文件操作作好
       准备.
***********************************************************************************/
uint8_t znFAT_Init(void)
{
    struct DBR *pdbr;

    uint8_t dm[3] = DBR_MARK;
    ...
    if(!IS_FAT32_TYPE((pdbr->BS_FilSysType1))) //FAT32文件系统类型检验
    {
        return FSTYPE_NOT_FAT32;
    }
...
#ifndef RT_UPDATE_CLUSTER_CHAIN
#ifndef USE_ALONE_CCCB
    Memory_Set((uint8_t *)pcccb_buf, sizeof(uint32_t)*CCCB_LEN, 0);
#endif
#endif
...
uint8_t CCCB_To_Alone(void) //定向到当前文件的CCCB
{
...
uint32_t CCCB_Get_Next_Cluster(uint32_t cluster)
{
    uint32_t pos = CCCB_LEN - 1;
    uint32_t i = 0, temp = 0;

    if(pcccb_buf == (uint32_t *)0) return 0;
...
uint8_t CCCB_Update_FAT(void)
{
    uint32_t i = 0, j = 0, temp = 0, temp1 = 0;
    uint32_t old_clu = 0, cur_clu = 0, clu_sec = 0;

#ifndef USE_ALONE_CCCB
    uint8_t old_devno = Dev_No;
    ...
    for(i = 0; i < temp1;)
    {
        for(j = pcccb_buf[2 * i] + 1; j <= pcccb_buf[2 * i + 1]; j++)
        {
            ...
            temp = (uint8_t)(old_clu % NITEMSINFATSEC);
            (((pFAT_Sec->items)[temp]).Item)[0] = (uint8_t)(cur_clu & 0X000000FF)      ; //将其链在前面的簇项上
            (((pFAT_Sec->items)[temp]).Item)[1] = (uint8_t)((cur_clu & 0X0000FF00) >> 8) ;
            (((pFAT_Sec->items)[temp]).Item)[2] = (uint8_t)((cur_clu & 0X00FF0000) >> 16);
            (((pFAT_Sec->items)[temp]).Item)[3] = (uint8_t)((cur_clu & 0XFF000000) >> 24);
...
//============================================================================================
    Memory_Set((uint8_t *)pcccb_buf, sizeof(uint32_t)*CCCB_LEN, 0); //清空CCCB
    (*pcccb_counter) = 0;

#ifndef USE_ALONE_CCCB
    pcccb_cur_oc = (struct FileInfo *)0;
    *pcccb_curdev = (uint8_t)(-1);
    pcccb_cur_initargs = (struct znFAT_Init_Args *)0;

    Dev_No = old_devno;
    pInit_Args = old_pinit_args; //恢复设备号与 与设备相关的文件系统参数集合
#endif
...
uint32_t Get_Next_Cluster(uint32_t cluster)
{
    ...
    return Bytes2Value((uint8_t *)pFAT_Item, NFATITEMBYTES); //返回簇项的值, 即指定簇下一簇的簇号
}
#endif
...
***********************************************************************************/
#ifdef ZNFAT_SEEK
uint8_t znFAT_Seek(struct FileInfo *pfi, uint32_t offset)
{
    uint32_t Cluster_Size = ((pInit_Args->SectorsPerClust) * (pInit_Args->BytesPerSector)); //计算簇的总字节数据, 以免后面重复计算
    uint32_t temp = 0, temp1 = 0, temp2 = 0, len = 0, k = 0, ncluster = 0, have_read = 0;
            ...
            if(!(IS_END_SEC_OF_CLU(pfi->File_CurSec, pfi->File_CurClust))) //如果当前扇区不是当前簇的最后一个扇区
            {
                ...
                if(temp2 <= temp1) //如果剩余数据量xxx
                {
                    ...
                    else
                    {
                        ...
                        //更新当前位置参数
                        pfi->File_CurPos = (uint16_t)(len - have_read);
                    }
                    pfi->File_CurOffset += len;
...
uint32_t znFAT_ReadData(struct FileInfo *pfi, uint32_t offset, uint32_t len, uint8_t *app_Buffer)
{
    uint32_t Cluster_Size = 0, iClu = 0, next_clu = 0, start_clu = 0, end_clu = 0;
    uint32_t temp = 0, temp1 = 0, temp2 = 0, ncluster = 0, have_read = 0;
...
uint32_t znFAT_ReadDataX(struct FileInfo *pfi, uint32_t offset, uint32_t len) //数据重定向读取套用了数据读取函数, 但并不使用多扇区驱动
{
    //加之没有足够的RAM资源, 且每次处理一个字节, 因此效率不高
    uint32_t Cluster_Size = 0, iClu = 0, iSec = 0, next_clu = 0, start_clu = 0, end_clu = 0;
    uint32_t temp = 0, temp1 = 0, temp2 = 0, k = 0, ncluster = 0, have_read = 0;
    uint32_t i = 0;
...
uint8_t Check_Illegal_Char(int8_t *pfn)
{
    uint32_t i = 0;
    while(pfn[i])
    {
...
uint8_t Check_SFN_Special_Char(int8_t *pfn)
{
    uint32_t i = 0;
    uint32_t pos = (StringLen(pfn) - 1);
...
******************************************************************************************/
#ifdef CHECK_SFN_DOT
uint8_t Check_SFN_Dot(int8_t *pfn)
{
    uint32_t pos = (StringLen(pfn) - 1);
...
uint8_t Check_SFN_Illegal_Lower(int8_t *pfn)
{
    uint32_t i = (StringLen(pfn) - 1);
    uint8_t flag1 = 2, flag2 = 2;
...
uint8_t Check_SFN_Illegal_Length(int8_t *pfn)
{
    uint8_t dot_pos = 0, have_dot = 0, i = 0;
    uint8_t mainfn_len = 0, extfn_len = 0;
    uint32_t fn_len = StringLen(pfn);

    while(' ' == pfn[fn_len - 1]) fn_len--;

    if(fn_len > 12) return ERR_FAIL; //fn is longer than 8.3

    for(i = (uint8_t)(fn_len - 1); i > 0; i--) //反向寻找. 第一个.是主文件与扩展名的分界
    {
        if('.' == pfn[i])
        {
            dot_pos = i;
    ...
    {
        mainfn_len = dot_pos;
        extfn_len = (uint8_t)(((uint8_t)fn_len) - (uint8_t)(dot_pos + 1));
...
******************************************************************************************/
#ifdef TO_FILE_NAME
uint8_t To_File_Name(int8_t *name_in_fdi, int8_t *pfileName)
{
    uint8_t i = 0, n = 7, m = 10;
...
******************************************************************************************/
#ifdef ANALYSE_FDI
uint8_t Analyse_FDI(struct FileInfo *pfi, struct FDI *pFDI)
{
    uint32_t temp = 0, i = 0;
...
//==========================================================
    ...
    To_File_Name((int8_t *)pFDI, pfi->File_Name);
    ...
    while('.' != (pfi->File_Name)[temp] && temp > 0)
    {
        ...
        {
            (pfi->File_Name)[temp] = (int8_t)Upper2Low((pfi->File_Name)[temp]);
        }
        temp--;
    }
    if(((pFDI->LowerCase) & 0x08) != 0)
    {
        for(i = 0; i < temp; i++)
        {
            (pfi->File_Name)[i] = (int8_t)Upper2Low((pfi->File_Name)[i]);
        }
    }
...
//解析文件创建时间与日期
    ...
    pfi->File_CTime.sec = (uint8_t)((temp & TIME_SEC_MARK) * 2);
    temp >>= TIME_SEC_NBITS; //创建时间的2秒位
    pfi->File_CTime.min = (uint8_t)(temp & TIME_MIN_MARK);
    temp >>= TIME_MIN_NBITS; //创建时间的分位
    pfi->File_CTime.hour = (uint8_t)(temp & TIME_HOUR_MARK); //创建时间的时位
    pfi->File_CTime.sec += (uint8_t)((uint16_t)(pFDI->CTime10ms) / 100); //在秒上加上10毫秒位

    temp = Bytes2Value(pFDI->CDate, 2);
    pfi->File_CDate.day = (uint8_t)(temp & DATE_DAY_MARK);
    temp >>= DATE_DAY_NBITS; //创建日期的日位
    pfi->File_CDate.month = (uint8_t)(temp & DATE_MONTH_MARK);
    temp >>= DATE_MONTH_NBITS; //创建日期的月位
    pfi->File_CDate.year = (uint16_t)((temp & DATE_YEAR_MARK) + DATE_YEAR_BASE); //创建日期的年位（加上年份基数）

//解析文件修改时间与日期
//temp=Bytes2Value(pFDI->MTime,2);
//pfi->File_MTime.sec=(uint8_t)((temp&TIME_SEC_MARK)*2); temp>>=TIME_SEC_NBITS;  //创建时间的2秒位
//pfi->File_MTime.min=(uint8_t)(temp&TIME_MIN_MARK);   temp>>=TIME_MIN_NBITS; //创建时间的分位
//pfi->File_MTime.hour=(uint8_t)(temp&TIME_HOUR_MARK); //创建时间的时位
//文件的修改时间没有10毫秒位, 所以它只能表达偶数秒

//temp=Bytes2Value(pFDI->MDate,2);
//pfi->File_MDate.day=(uint8_t)(temp&DATE_DAY_MARK);     temp>>=DATE_DAY_NBITS;   //创建日期的日位
//pfi->File_MDate.month=(uint8_t)(temp&DATE_MONTH_MARK); temp>>=DATE_MONTH_NBITS; //创建日期的月位
//pfi->File_MDate.year=(uint8_t)((temp&DATE_YEAR_MARK)+DATE_YEAR_BASE); //创建日期的年位

//解析文件访问日期
//temp=Bytes2Value(pFDI->ADate,2);
//pfi->File_ADate.day=(uint8_t)(temp&DATE_DAY_MARK);     temp>>=DATE_DAY_NBITS;   //创建日期的日位
//pfi->File_ADate.month=(uint8_t)(temp&DATE_MONTH_MARK); temp>>=DATE_MONTH_NBITS; //创建日期的月位
//pfi->File_ADate.year=(uint8_t)((temp&DATE_YEAR_MARK)+DATE_YEAR_BASE); //创建日期的年位

    pfi->File_CurClust = pfi->File_StartClust;
    pfi->File_CurSec = (pfi->File_CurClust) ? SOC(pfi->File_CurClust) : 0;
    pfi->File_CurPos = 0;
    pfi->File_CurOffset = 0;
    pfi->File_IsEOF = (uint8_t)((pfi->File_StartClust) ? BOOL_FALSE : BOOL_TRUE);
...
uint8_t Is_WildFileName(int8_t *pfn)
{
    uint8_t i = 0;
    while('\0' != pfn[i])
    {
...
******************************************************************************************/
#ifdef FINDSUBSTR
uint8_t FindSubStr(int8_t *str, int8_t *substr, uint8_t pos)
{
    uint8_t i = pos, j = 0, lens = (uint8_t)StringLen(str), lent = (uint8_t)StringLen(substr);
    ...
    if(j == lent) return (uint8_t)(i - lent);
...
**********************************************************************************************/
#ifdef SFN_MATCH
uint8_t SFN_Match(int8_t *t, int8_t *s)
{
    uint8_t i = 0, j = 0, lens = (uint8_t)StringLen(s);
    uint8_t lent = (uint8_t)StringLen(t);
    int8_t buf[20];
    uint8_t bufp = 0;
...
**********************************************************************************************/
#ifdef MODIFY_FAT
uint8_t Modify_FAT(uint32_t cluster, uint32_t next_cluster) //构造FAT簇链
{
    ...
    znFAT_Buffer[temp2 + 0] = (uint8_t)( next_cluster & 0x000000ff)     ;
    znFAT_Buffer[temp2 + 1] = (uint8_t)((next_cluster & 0x0000ff00) >> 8 );
    znFAT_Buffer[temp2 + 2] = (uint8_t)((next_cluster & 0x00ff0000) >> 16);
    znFAT_Buffer[temp2 + 3] = (uint8_t)((next_cluster & 0xff000000) >> 24);
...
********************************************************************************************/
#ifdef CLEAR_CLUSTER
uint8_t Clear_Cluster(uint32_t cluster)
{
...
uint8_t Update_Next_Free_Cluster(void)
{
    uint32_t clu_sec, iItem, iSec;
    struct FAT_Sec *pFAT_Sec;
...
********************************************************************************************/
#ifdef IS_LFN
uint8_t Is_LFN(int8_t *filename)
{
    uint8_t is_lfn = BOOL_FALSE;
    ...
    if(((uint8_t)(-1)) == Check_SFN_Illegal_Lower(filename)) is_lfn = BOOL_TRUE;
...
************************************************************************************************/
#ifdef GET_PART_NAME
uint8_t Get_Part_Name(uint16_t *lfn_buf, struct LFN_FDI *plfndi, uint8_t n)
{
    uint8_t i = 0;
    uint16_t temp = 0;

    if((plfndi->AttrByte[0]) & 0X40) Memory_Set(((uint8_t *)lfn_buf), 2 * (MAX_LFN_LEN + 1), 0); //如果是最后一个长名项, 则清空lfn_buf

    for(i = 0; i < 5; i++) //第一部分长名
    {
        if(n >= MAX_LFN_LEN) return ERR_LFN_BUF_OUT;
        temp = (((uint16_t)(((plfndi->Name1) + i * 2)[0])) & 0X00FF) | ((((uint16_t)(((plfndi->Name1) + i * 2)[1])) & 0X00FF) << 8);
        if(0 == temp)
        {
            lfn_buf[n] = 0;
            return 0;
    ...
    {
        if(n >= MAX_LFN_LEN) return ERR_LFN_BUF_OUT;
        temp = (((uint16_t)(((plfndi->Name2) + i * 2)[0])) & 0X00FF) | ((((uint16_t)(((plfndi->Name2) + i * 2)[1])) & 0X00FF) << 8);
        if(0 == temp)
        {
            lfn_buf[n] = 0;
            return 0;
    ...
    {
        if(n >= MAX_LFN_LEN) return ERR_LFN_BUF_OUT;
        temp = (((uint16_t)(((plfndi->Name3) + i * 2)[0])) & 0X00FF) | ((((uint16_t)(((plfndi->Name3) + i * 2)[1])) & 0X00FF) << 8);
        if(0 == temp)
        {
            lfn_buf[n] = 0;
            return 0;
    ...
    {
        while(((uint16_t)(0XFFFF)) == lfn_buf[n]) n--;
        lfn_buf[n + 1] = 0;
    }
...
************************************************************************************************/
#ifdef OEMTOUNI
uint8_t OEM2UNI(uint16_t oem_code, uint16_t *uni_code) //通过二分法查表将OEM码转为UNI码
{
    uint32_t low = 0, high = MAX_UNI_INDEX - 1, mid; //置当前查找区间上下界的初值
...
uint8_t oemstr2unistr(int8_t *oemstr, uint16_t *unistr)
{
    uint32_t len = StringLen(oemstr);
    uint32_t i = 0, pos = 0;
    uint8_t res = 0;
    uint16_t temp = 0;
    ...
    for(i = 0; i < len; i++)
    {
        ...
        {
            unistr[pos] = (uint16_t)(((uint16_t)oemstr[i]) & 0X00FF);
            pos++;
        }
#ifdef USE_OEM_CHAR
        else //不是ASCII码, 而是OEM编码
        {
            temp = ((((uint16_t)oemstr[i]) << 8) & 0xff00);
            temp |= (((uint16_t)oemstr[i + 1]) & 0x00ff);
...
************************************************************************************************/
#ifdef WFINDSUBSTR
uint32_t WFindSubStr(uint16_t *str, uint16_t *substr, uint32_t pos)
{
    uint32_t i = pos, j = 0, lens = WStringLen(str), lent = WStringLen(substr);

    while(i < lens && j < lent)
    {
        if(WLower2Up(str[i]) == substr[j] || (uint16_t)'?' == substr[j])
        {
            i++;
            j++;
        }
...
************************************************************************************************/
#ifdef LFN_MATCH
uint8_t LFN_Match(uint16_t *t, uint16_t *s)
{
    uint32_t i = 0, j = 0, lens = WStringLen(s), lent = WStringLen(t);
    uint16_t bufp = 0;

    uint16_t buf[MAX_LFN_LEN + 1];

//======================================================

    while(j < lent && (uint16_t)'*' != t[j])
    {
    ...
    while(1)
    {
        while(j < lent && (uint16_t)'*' == t[j]) j++;
        if(j == lent) return ERR_SUCC;
        bufp = 0;

        while(j < lent && (uint16_t)'*' != t[j])
        {
            buf[bufp] = (uint16_t)WLower2Up(t[j]);
            bufp++;
            j++;
        }
        buf[bufp] = 0;
        ...
        if(((uint32_t)(-1)) == i) return ERR_FAIL;
        i += bufp;
    }
}
#endif
...
uint8_t Get_Binding_SumChk(struct FDI *pdi)
{
    uint8_t i = 0, c = 0;

    for(i = 0; i < 11; i++)
    {
        c = (uint8_t)(((c & 0X01) ? 0X80 : 0) + (c >> 1) + ((pdi->Name)[i]));
    }
...
************************************************************************************************/
#ifdef HEX2STR_32B
uint8_t Hex2Str_32b(uint32_t hex, int8_t *str)
{
    uint8_t i = 0, temp = 0;
    for(i = 0; i < 8; i++)
    {
        temp = ((unsigned char)((hex & (0X0000000F << (i * 4))) >> (i * 4)));
        str[7 - i] = (int8_t)((temp >= 10) ? ('A' + temp - 10) : (temp + 0X30));
    }
    str[i] = 0;
...
uint8_t Make_Short_Name(char *pfn, char *psfn) //此函数用于生成长名对应的短名, 使用了ELFHASH算法用于生成唯一的短名
{
//psfn[0]=Lower2Up(pfn[0]);
//psfn[1]='~';
    uint32_t temp = ELFHash(pfn);
...
*******************************************************************************/
#ifdef FILL_LFN_FDI
uint8_t Fill_LFN_FDI(uint16_t *unifn, struct LFN_FDI *plfni, uint8_t ChkSum, uint8_t n)
{
    uint8_t temp = (uint8_t)(n * 13), i = 0, j = 0;
    uint16_t len = (uint16_t)WStringLen(unifn + temp);

    (plfni->AttrByte[0]) = (uint8_t)(n + 1); //长名项的序号字节
    ...
    for(i = 0; i < 5; i++)
    {
        (plfni->Name1)[2 * i] = (uint8_t)(unifn[j + temp] & 0X00FF);
        (plfni->Name1)[2 * i + 1] = (uint8_t)((unifn[j + temp] & 0XFF00) >> 8);
        j++;
        if(j >= len)
        {
            if(4 == i)
    ...
    for(i = 0; i < 6; i++)
    {
        (plfni->Name2)[2 * i] = (uint8_t)(unifn[j + temp] & 0X00FF);
        (plfni->Name2)[2 * i + 1] = (uint8_t)((unifn[j + temp] & 0XFF00) >> 8);
        j++;
        if(j >= len)
        {
            if(5 == i)
    ...
    for(i = 0; i < 2; i++)
    {
        (plfni->Name3)[2 * i] = (uint8_t)(unifn[j + temp] & 0X00FF);
        (plfni->Name3)[2 * i + 1] = (uint8_t)((unifn[j + temp] & 0XFF00) >> 8);
        j++;
        if(j >= len)
        {
            if(1 != i)
...
uint8_t Register_LFN_FDI(uint32_t cluster, struct FDI *pfdi, uint16_t *unifn, uint32_t *psec, uint8_t *pn)
{
    uint32_t temp_sec = 0, old_cluster = 0;
    uint8_t iClu = 0, iSec = 0, iFDI = 0, res = 0;
    struct FDIesInSEC *pitems;
    struct FDI *pitem;

    struct LFN_FDI *plfni;

    uint8_t have_lfn = 0; //在文件目录项扫描过程种, 用于标记是否有长名
    uint8_t is_lfn_buf_err = 0, cur_binding_sumchk = 0, flag = 0, chksum = Get_Binding_SumChk(pfdi);
    uint16_t lfn_buf[MAX_LFN_LEN + 1]; //用于装载长名UNI码的缓冲
    struct LFN_FDI temp_lfni; //用于构造长名项的临时结构体变量

    uint16_t len = (uint16_t)WStringLen(unifn), xlen = 0; //计算UNI串长度
    uint8_t nclu = 0, nsec = 0, nlfni = (uint8_t)(len / 13);
...
//以下代码检查是否已经存在重复的文件或目录, 以及查询空位并填入目录项
    do
    {
        ...
        {
            znFAT_Device_Read_Sector(temp_sec + (uint32_t)iSec, znFAT_Buffer);
            ...
            for(iFDI = 0; iFDI < NFDI_PER_SEC; iFDI++)
            {
                        ...
                        && (0XE5 != pitem->Name[0]) && ('.' != pitem->Name[0])) //即遇到一个合法的SFN项
                {
                    if(have_lfn && !is_lfn_buf_err) //遇到SFN项后, 它前面可能有长名项, 如果有则与输入的UNI串比对, 以确定是否有同名文件或目录
                    {
                        ...
                        {
                            xlen = (uint16_t)WStringLen(lfn_buf); //计算从各长名项合成后的UNI串的长度
                            if(xlen == len && Memory_Compare(((uint8_t *)unifn), ((uint8_t *)lfn_buf), 2 * ((uint32_t)len))) //对UNI串进行比对, 长度和内容要相等
                            {
                                *psec = temp_sec + (uint32_t)iSec;
                ...
                if(CHK_ATTR_LFN(pitem->Attributes) && (0XE5 != pitem->Name[0])) //是长名项, 而且没有被删除
                {
                    ...
                    res = Get_Part_Name(lfn_buf, plfni, (uint8_t)((((plfni->AttrByte[0]) & 0XBF) - 1) * 13)); //将当前LFN项中的文件名UNICODE码拼入临时缓冲
                ...
                if(0 == (pitem->Name)[0])
                {
                    ...
                    if(nlfni > 0)
                    {
                        Fill_LFN_FDI(unifn, &temp_lfni, chksum, (uint8_t)(nlfni - 1));
                        Memory_Copy(((uint8_t *)pitem), ((uint8_t *)(&temp_lfni)), FDI_NBYTES); //将合成的长名项拷贝到扇区数据中
                        nlfni--;
                    }
                    else
                    {
                        *psec = temp_sec + (uint32_t)iSec;
                        *pn = iFDI; //记录空位的位置
                        Memory_Copy(((uint8_t *)pitem), ((uint8_t *)pfdi), FDI_NBYTES); //将短名项拷贝到扇区数据中
                        znFAT_Device_Write_Sector(temp_sec + (uint32_t)iSec, znFAT_Buffer);
                        return ERR_SUCC;
                    }
                }
            }
...
//如果运行到这里, 则说明当前簇中已无空位
    ...
    {
        nsec = (uint8_t)((nlfni + 1) / NFDI_PER_SEC); //剩余的长名项还需要多少个扇区,+1是因为最后还有一个短名项
        if((nlfni + 1) % NFDI_PER_SEC) nsec++;

        nclu = (uint8_t)(nsec / (pInit_Args->SectorsPerClust));
...
uint8_t znFAT_Open_File(struct FileInfo *pfi, int8_t *filepath, uint32_t n, uint8_t is_file)
{
    uint8_t result = 0, flag = 0;
    uint32_t sec_temp = 0, Cur_Cluster = 0, fn_pos = 0, item = 0;
    uint32_t iSec = 0, iFDI = 0;

//===========for LFN=======
#ifdef USE_LFN
    struct LFN_FDI *plfni;

    uint8_t cur_binding_sumchk = 0;
    uint8_t is_wfn = 0;
    uint16_t temp_lfn[MAX_LFN_LEN + 1]; //用于装载长名UNICODE码的临时缓冲
    uint8_t is_lfn = 0; //输入的文件名是否是长名
    uint8_t is_lfn_buf_err = 0; //产生长名缓冲溢出错误, 此标记为1则直接跳过长名比对
#endif
//===========for LFN=======

    int8_t temp_filename[13];
    int8_t *filename;
...
//这里主要针对于LFN、SFN及通配名进行相关检测
    if(!Is_WildFileName(filename)) //如果不是通配文件名, 即确定名, 则需要进行文件名合法性检测
    {
        ...
        if(!Is_LFN(filename)) //如果不是长名,即是短名
        {
            ...
            if(((uint8_t)(-1)) == Check_SFN_Illegal_Lower(filename)) return ERR_SFN_ILL_LOWER; //检查SFN中是否有非法的大小写
...
//================================================
    do
    {
        ...
        for(iSec = 0; iSec < (pInit_Args->SectorsPerClust); iSec++)
        {
            znFAT_Device_Read_Sector(sec_temp + (uint32_t)iSec, znFAT_Buffer);
            pitems = (struct FDIesInSEC *)znFAT_Buffer;
            ...
            for(iFDI = 0; iFDI < NFDI_PER_SEC; iFDI++) //访问扇区中各文件目录项
            {
                ...
                if((is_file ? CHK_ATTR_FILE(pitem->Attributes) : CHK_ATTR_DIR(pitem->Attributes))
                    ...
                    //不是.与..
                {
                    To_File_Name((int8_t *)(pitem->Name), temp_filename); //将FDI中的文件名字段转为8.3文件名
                        ...
                        //算比对, 结果也一定是不匹配的
                    {
                        if(!SFN_Match(filename, temp_filename)) //短文件名通配
                        {
                            if(n == item)
                            {
                                ...
                                pfi->nFDI = (uint8_t)iFDI; //文件目录项在扇区中的索引
                    ...
                    if(flag)
                    {
                        item++;    //如果LFN（如果用了LFN的话）与SFN有一项比对成功, 但item不匹配, 则item++
                        flag = 0;
                    }
                }
                ...
                if(CHK_ATTR_LFN(pitem->Attributes) && (0XE5 != pitem->Name[0]) && is_lfn) //是长名项, 而且没有被删除, 并且输入的文件名是长名
                {
                    ...
                    result = Get_Part_Name(pfi->longname, plfni, (uint8_t)((((plfni->AttrByte[0]) & 0XBF) - 1) * 13)); //将当前LFN项中的文件名UNICODE码拼入临时缓冲
                    //此临时缓冲长度为MAX_LFN_LEN, 如果越界, 则
                    //不再装入, 最终造成LFN的截断.
...
uint8_t Get_Dir_Start_Cluster(int8_t *dir_name, uint32_t *pCluster) //获取目录下名为dir_name的子目录的开始簇
{
    //目录的首簇存入pCluster所指入的变量
    uint32_t sec_temp = 0;
    uint8_t iSec = 0, iFDI = 0;
    uint32_t Cur_Clust = *pCluster;

//===========for LFN=======
#ifdef USE_LFN
    struct LFN_FDI *plfni;

    uint8_t cur_binding_sumchk = 0, result = 0;
    uint16_t temp_lfn[MAX_LFN_LEN + 1]; //用于装载长名UNICODE码的临时缓冲
    uint16_t lfn_buf[MAX_LFN_LEN + 1]; //从长名项中提取的UNI码装到这个缓冲中
    uint8_t is_lfn = 0; //输入的文件名是否是长名
    uint8_t have_lfn = 0; //在搜索目录项时发现了长名项
    uint8_t is_lfn_buf_err = 0; //产生长名缓冲溢出错误, 此标记为1则直接跳过长名比对
#endif

//===========for LFN=======

    int8_t temp_dirname[13];
    ...
    if(!Is_LFN(dir_name)) //如果不是长名,即是短名
    {
        ...
        if(((uint8_t)(-1)) == Check_SFN_Illegal_Lower(dir_name)) return ERR_SFN_ILL_LOWER; //检查SFN中是否有非法的大小写
#ifdef USE_LFN
    }
    else //如果是长名
    {
    ...
    do
    {
        ...
        for(iSec = 0; iSec < (pInit_Args->SectorsPerClust); iSec++)
        {
            ...
            for(iFDI = 0; iFDI < NFDI_PER_SEC; iFDI++) //访问扇区中各文件目录项
            {
                ...
                {
                    To_File_Name((int8_t *)(pitem->Name), temp_dirname); //将FDI中的目录名字段转为8.3文件名
#ifdef USE_LFN
                    if(!is_lfn)
#endif
                    {
                ...
                if(CHK_ATTR_LFN(pitem->Attributes) && (0XE5 != pitem->Name[0]) && is_lfn) //是长名项, 而且没有被删除,输入的名子为长名
                {
                    ...
                    result = Get_Part_Name(lfn_buf, plfni, (uint8_t)((((plfni->AttrByte[0]) & 0XBF) - 1) * 13)); //将当前LFN项中的文件名UNICODE码拼入临时缓冲
                    //此临时缓冲长度为MAX_LFN_LEN, 如果越界, 则
                    //不再装入, 最终造成LFN的截断.
...
uint8_t znFAT_Enter_Dir(int8_t *dirpath, uint32_t *pCluster, uint32_t *pos)
{
    uint8_t index = 0, res = 0;
    uint32_t i = 1;

#ifndef USE_LFN
    int8_t dirname[13];
#else
    int8_t dirname[MAX_LFN_LEN + 1];
#endif
...
*************************************************************************************/
#ifdef FILL_FDI
uint8_t Fill_FDI(struct FDI *pfdi, int8_t *pfn, struct DateTime *pdt, uint8_t is_file)
{
    uint8_t dot_pos = 0, have_dot = 0, lowcase = 0;
    uint8_t i = 0, j = 0, fn_len = (uint8_t)StringLen(pfn);
    uint16_t time = 0, date = 0;

    Memory_Set(((uint8_t *)pfdi), FDI_NBYTES, 0); //对文件目录项清0
    ...
    if(have_dot) //如果有点
    {
        ...
        for(i = 0; i < dot_pos; i++)
        {
            (pfdi->Name)[i] = (int8_t)Lower2Up(pfn[i]); //转为大写
        }
        for(; i < 8; i++)
        {
        ...
        //填入扩展名
        for(i = (uint8_t)(dot_pos + 1); i < fn_len; i++)
        {
            (pfdi->Extension)[j] = (uint8_t)Lower2Up(pfn[i]); //转为大写
            j++;
        }
        for(; j < 3; j++)
        {
    ...
    else //如果没有点
    {
        ...
        for(i = 0; i < fn_len; i++)
        {
            (pfdi->Name)[i] = (uint8_t)Lower2Up(pfn[i]); //转为大写
        }
        for(; i < 8; i++)
        {
            (pfdi->Name)[i] = ' '; //不足8字节部分填入空格
...
//=======================填入主文件名与扩展名========
    ...
    pfdi->Attributes = (uint8_t)(is_file ? 0X20 : 0X30); //设置属性
//=======================填入属性========
...
//=======================填入大小写控制字============
    ...
    pfdi->CTime10ms = (uint8_t)((((pdt->time).sec) % 2) ? 0X78 : 0X00);
//=======================填入创建时间10MS位==========

    time = (uint16_t)(MAKE_TIME((pdt->time).hour, (pdt->time).min, (pdt->time).sec));
    (pfdi->CTime)[0] = (uint8_t)time;
    (pfdi->CTime)[1] = (uint8_t)(time >> 8);
//=======================填入创建时间================

    date = (uint16_t)(MAKE_DATE((pdt->date).year, (pdt->date).month, (pdt->date).day));
    (pfdi->CDate)[0] = (uint8_t)date;
    (pfdi->CDate)[1] = (uint8_t)(date >> 8);
//=======================填入创建日期================

    (pfdi->ADate)[0] = (uint8_t)date;
    (pfdi->ADate)[1] = (uint8_t)(date >> 8);
//=======================填入访问日期================

    (pfdi->MTime)[0] = (uint8_t)time;
    (pfdi->MTime)[1] = (uint8_t)(time >> 8);
//=======================填入修改时间================

    (pfdi->MDate)[0] = (uint8_t)date;
    (pfdi->MDate)[1] = (uint8_t)(date >> 8);
//=======================填入修改日期================

//填充开始簇
    if(!is_file) //如果是目录, 则在fdi中填充空簇
    {
        pfdi->HighClust[0] = (uint8_t)(((pInit_Args->Next_Free_Cluster) >> 16) & 0X000000FF); //目录与文件不同, 目录在创建之初
        pfdi->HighClust[1] = (uint8_t)(((pInit_Args->Next_Free_Cluster) >> 24) & 0X000000FF); //就要为其分配空簇, 为的是写入.与..
        pfdi->LowClust [0] = (uint8_t)(((pInit_Args->Next_Free_Cluster)    ) & 0X000000FF); //这两个特殊的文件目录项
        pfdi->LowClust [1] = (uint8_t)(((pInit_Args->Next_Free_Cluster) >> 8 ) & 0X000000FF);
    }
...
*************************************************************************************/
#ifdef REGISTER_FDI
uint8_t Register_FDI(uint32_t cluster, struct FDI *pfdi, uint32_t *psec, uint8_t *pn)
{
    uint32_t temp_sec = 0, old_cluster = 0;
    uint8_t iSec = 0, iFDI = 0;
...
//以下代码检查是否已经存在重复的文件目录项, 以及查询空位
    do
    {
        ...
        for(iSec = 0; iSec < (pInit_Args->SectorsPerClust); iSec++)
        {
            ...
            for(iFDI = 0; iFDI < NFDI_PER_SEC; iFDI++)
            {
                if(Memory_Compare((uint8_t *)((pfdis->FDIes) + iFDI), (uint8_t *)pfdi, 11)) //比较文件名
                {
                    *psec = temp_sec + (uint32_t)iSec;
                    *pn = iFDI; //记录重名文件目录项的位置, 以便对其进行解析
                ...
                else
                {
                    if(0 == ((((pfdis->FDIes)[iFDI]).Name)[0]))
                    {
                        ...
                        *pn = iFDI; //记录空位的位置
                        ...
                        Memory_Copy((uint8_t *)((((struct FDIesInSEC *)znFAT_Buffer)->FDIes) + *pn), (uint8_t *)pfdi, FDI_NBYTES);
...
//如果运行到这里, 则说明当前簇中已无空位
    if(0 != pInit_Args->Free_nCluster) //如果剩余空簇数为0, 则说明磁盘已无空间
    {
        ...
        *pn = 0; //记录空位的位置
        ...
        znFAT_Device_Read_Sector(*psec, znFAT_Buffer);
        Memory_Copy((uint8_t *)((((struct FDIesInSEC *)znFAT_Buffer)->FDIes)), (uint8_t *)pfdi, FDI_NBYTES);
        znFAT_Device_Write_Sector(*psec, znFAT_Buffer);
...
uint8_t znFAT_Create_File(struct FileInfo *pfi, int8_t *pfn, struct DateTime *pdt)
{
    uint32_t Cur_Cluster = 0, pos = 0, sec = 0;
    uint8_t res = 0, n = 0;
    struct FDI fdi;
    int8_t *filename;
...
uint8_t Create_Dir_In_Cluster(uint32_t *cluster, int8_t *pdn, struct DateTime *pdt)
{
    uint8_t res = 0, i = 0;
    uint32_t dummy = 0;
    struct FDI fdi;
...
#ifdef USE_LFN
    uint8_t is_lfn = 0; //标记输入的目录名是否长名
    uint16_t temp_lfn_buf[MAX_LFN_LEN + 1]; //用于装载长名UNI串的临时缓冲
    int8_t temp_dirname[13];
#endif
    ...
    if(!Is_LFN(pdn))
    {
        ...
        if(((uint8_t)(-1)) == Check_SFN_Illegal_Lower(pdn)) return ERR_SFN_ILL_LOWER; //检查SFN中是否有非法的大小写
#ifdef USE_LFN
    }
    else
    {
...
//把fdi中的名字替换为. 名为.的目录指向了相前簇
    ...
    Memory_Copy(znFAT_Buffer, ((uint8_t *)(&fdi)), FDI_NBYTES); //将文件目录项.装入到内部缓冲区中
...
//把fdi中的簇值改为上一级目录的首簇
    ...
    {
        fdi.HighClust[0] = (uint8_t)(((*cluster) >> 16) & 0X000000FF);
        fdi.HighClust[1] = (uint8_t)(((*cluster) >> 24) & 0X000000FF);
        fdi.LowClust [0] = (uint8_t)(((*cluster)    ) & 0X000000FF);
        fdi.LowClust [1] = (uint8_t)(((*cluster) >> 8 ) & 0X000000FF);
    }
    else
    {
    ...
    Memory_Copy(znFAT_Buffer + FDI_NBYTES, ((uint8_t *)(&fdi)), FDI_NBYTES); //将文件目录项..装入到内部缓冲区中
...
uint8_t znFAT_Create_Dir(int8_t *pdp, struct DateTime *pdt)
{
    uint32_t Cur_Cluster = 0, i = 0;
    uint8_t index = 0, res = 0;

#ifndef USE_LFN
    uint8_t dirname[13];
#else
    uint8_t dirname[MAX_LFN_LEN + 1];
#endif
    ...
    if(znFAT_Enter_Dir(pdp, &Cur_Cluster, &i)) //进入目录, 如果中途发生错误
    {
        ...
        while('\0' != pdp[i])                   //目录不存在, 则将最近的一层
        {
            ...
            if('\\' == pdp[i] || '/' == pdp[i])                   //而将失败的那一级目录名的第一
            {
                ...
                res = Create_Dir_In_Cluster(&Cur_Cluster, (int8_t *)dirname, pdt); //在簇Cur_Cluster中创建目录
                if(res)
                {
                    return res;  //返回错误码
                }
...
uint8_t Destroy_FAT_Chain(uint32_t cluster)
{
    uint32_t clu_sec = 0, temp1 = 0, temp2 = 0, old_clu = 0, nclu = 1;
...
*************************************************************************************/
#ifdef DESTROY_FDI
uint8_t Destroy_FDI(struct FDI *pitem)
{
    uint32_t start_cluster = Bytes2Value(pitem->LowClust, 2) + Bytes2Value(pitem->HighClust, 2) * 65536;
...
uint8_t Have_Any_SubDir_With_Del_ForeFile(uint32_t *cluster, uint8_t for_del_dir)
{
    uint8_t iSec = 0, iFDI = 0;
    uint32_t sec_temp = 0;
        ...
        for(iSec = 0; iSec < (pInit_Args->SectorsPerClust); iSec++)
        {
            ...
            for(iFDI = 0; iFDI < NFDI_PER_SEC; iFDI++) //访问扇区中各文件目录项
            {
                ...
                if((CHK_ATTR_FILE(pitem->Attributes)) && (0XE5 != pitem->Name[0])) //文件属性为文件, 且没有被删除
                {
                    ...
                    znFAT_Device_Read_Sector(sec_temp + (uint32_t)iSec, znFAT_Buffer); //重新读取扇区数据, 恢复内部缓冲区的数据

                    pitem->Name[0] = 0XE5; //给文件目录项打上"已删除"的标记
                    pitem->HighClust[0] = pitem->HighClust[1] = 0; //开始簇的高字清0

                    znFAT_Device_Write_Sector(sec_temp + (uint32_t)iSec, znFAT_Buffer); //如果有销毁操作, 则回写扇区
                }
...
uint8_t Enter_Deep_Ahead_Dir(uint32_t *cluster)
{
...
*************************************************************************************/
#ifdef GET_UPPER_DIR
uint8_t Get_Upper_Dir(uint32_t *cluster)
{
    struct FDIesInSEC *pitems; //指向文件目录项扇区数据的指针
    struct FDI *pitem; //指向文件目录项数据的指针
...
uint8_t znFAT_Delete_Dir(int8_t *dirpath)
{
    uint32_t top_dir_first_cluster = 0, sub_dir_first_cluster = 0;
    uint8_t res = 0;
...
uint8_t znFAT_Delete_File(int8_t *filepath)
{
    uint8_t res = 0;
    struct FileInfo fi;
...
uint8_t Get_Recmd_szClu(uint32_t nsec)
{
...
*************************************************************************************/
#ifdef ZNFAT_MAKE_FS
uint8_t znFAT_Make_FS(uint32_t tt_sec, uint16_t clu_sz) //格式化 tt_sec 总扇区数 clu_sz 指定的簇大小
{
    ...
    tt_sec /= (uint32_t)(NSECPERCYLINDER);
    tt_sec *= (uint32_t)(NSECPERCYLINDER); //舍去“剩余扇区”，剩余扇区是指不足一个柱面的扇区数

//=================合成DBR扇区数据=============================================================
    PGM2RAM(znFAT_Buffer, _dbr, 512); //从模版数组中把数据搬到内部缓冲区
    pdbr = (struct DBR *)znFAT_Buffer;

    pdbr->BPB_SecPerClus = (uint8_t)(clu_sz / 512); //每簇扇区数
    ...
    pdbr->BPB_TotSec32[0] = (uint8_t)((tt_sec)    & 0X000000FF);
    pdbr->BPB_TotSec32[1] = (uint8_t)((tt_sec >> 8) & 0X000000FF);
    pdbr->BPB_TotSec32[2] = (uint8_t)((tt_sec >> 16) & 0X000000FF);
    pdbr->BPB_TotSec32[3] = (uint8_t)((tt_sec >> 24) & 0X000000FF); //该分区的总扇区数

    temp = (tt_sec - 32) / (((uint32_t)NITEMSINFATSEC) * ((uint32_t)(pdbr->BPB_SecPerClus)));
    if((tt_sec - 32) % ((uint32_t)NITEMSINFATSEC) * ((uint32_t)pdbr->BPB_SecPerClus)) temp++; //((tt_sec-32)-2*FATsz)/(SecPerClus*128)=FATsz 解这个等式
    temp2 = temp;

    pdbr->BPB_FATSz32[0] = (uint8_t)((temp)    & 0X000000FF);
    pdbr->BPB_FATSz32[1] = (uint8_t)((temp >> 8) & 0X000000FF);
    pdbr->BPB_FATSz32[2] = (uint8_t)((temp >> 16) & 0X000000FF);
    pdbr->BPB_FATSz32[3] = (uint8_t)((temp >> 24) & 0X000000FF); //FAT表的扇区数
...
//===================================以下代码完成对FSINFO扇区数据的合成========================
    ...
    temp = (tt_sec - 32 - 2 * temp) / ((uint32_t)(temp1)) - 1; //总簇数-1, 因为第2簇为首目录, 已经被卷标占用
    pfsinfo->Free_Cluster[0] = (uint8_t)((temp)    & 0X000000FF);
    pfsinfo->Free_Cluster[1] = (uint8_t)((temp >> 8) & 0X000000FF);
    pfsinfo->Free_Cluster[2] = (uint8_t)((temp >> 16) & 0X000000FF);
    pfsinfo->Free_Cluster[3] = (uint8_t)((temp >> 24) & 0X000000FF); //剩余空簇数
...
uint8_t Update_File_Size(struct FileInfo *pfi) //更新文件大小
{
    struct FDI *pfdi;
    ...
    (pfdi->FileSize)[0] = (uint8_t)((pfi->File_Size) & 0X000000FF)      ;
    (pfdi->FileSize)[1] = (uint8_t)(((pfi->File_Size) & 0X0000FF00) >> 8) ;
    (pfdi->FileSize)[2] = (uint8_t)(((pfi->File_Size) & 0X00FF0000) >> 16);
    (pfdi->FileSize)[3] = (uint8_t)(((pfi->File_Size) & 0XFF000000) >> 24);
...
uint8_t Update_File_sClust(struct FileInfo *pfi, uint32_t clu) //更新文件开始簇
{
    struct FDI *pfdi;
    ...
    (pfdi->HighClust)[0] = (uint8_t)((clu & 0X00FF0000) >> 16);
    (pfdi->HighClust)[1] = (uint8_t)((clu & 0XFF000000) >> 24);
    (pfdi->LowClust )[0] = (uint8_t)((clu & 0X000000FF))    ;
    (pfdi->LowClust )[1] = (uint8_t)((clu & 0X0000FF00) >> 8) ;
...
uint8_t Create_Cluster_Chain(struct FileInfo *pfi, uint32_t cluster, uint32_t datalen)
{
    uint32_t iSec = 0, clu_sec = 0, old_clu = cluster, ncluster = 0, temp_ncluster = 0;
    uint8_t iItem = 0, temp = 0;
    struct FAT_Sec *pFAT_Sec = (struct FAT_Sec *)znFAT_Buffer; //将数据缓冲区首地址强转为FAT_Sec结构体的指针类型

    uint32_t Clu_Size = (pInit_Args->SectorsPerClust * pInit_Args->BytesPerSector); //簇大小, 以免后面重复计算
...
//============================
    ...
    if(0 != old_clu)
    {
        ...
        temp = (uint8_t)(old_clu % NITEMSINFATSEC);
        (((pFAT_Sec->items)[temp]).Item)[0] = (uint8_t)(cluster & 0X000000FF)      ; //将其链在前面的簇项上
        (((pFAT_Sec->items)[temp]).Item)[1] = (uint8_t)((cluster & 0X0000FF00) >> 8) ;
        (((pFAT_Sec->items)[temp]).Item)[2] = (uint8_t)((cluster & 0X00FF0000) >> 16);
        (((pFAT_Sec->items)[temp]).Item)[3] = (uint8_t)((cluster & 0XFF000000) >> 24);
#else
        //---------------------------------CCCB的处理--------------------------------------
        if(0 == (*pcccb_counter))
        {
    ...
    if(clu_sec == (cluster / NITEMSINFATSEC)) //如果目标簇项与前一簇项在同一扇区
    {
        if(0 == ncluster)
        {
            temp = (uint8_t)(cluster % NITEMSINFATSEC);
...
//=============================================================================================
    ...
    if(((uint8_t)((cluster % NITEMSINFATSEC) + 1)) != ((uint8_t)NITEMSINFATSEC)) //如果当前的簇项不是其所在FAT扇区中的最后一个簇项
    {
        znFAT_Device_Read_Sector(clu_sec + (pInit_Args->FirstFATSector), znFAT_Buffer);

        for(iItem = (uint8_t)((cluster % NITEMSINFATSEC) + 1); iItem < NITEMSINFATSEC; iItem++) //检测在当前FAT扇区内当前簇项之后是否有空簇
        {
            cluster++; //簇号自增
...
uint32_t WriteData_From_nCluster(struct FileInfo *pfi, uint32_t len, uint8_t *pbuf)
{
    uint32_t CluSize = ((pInit_Args->BytesPerSector) * (pInit_Args->SectorsPerClust)); //计算簇大小, 以免后面重复计算
    uint32_t temp = len / CluSize; //计算要写入的数据量够几个整簇
    uint32_t iClu = 0, start_clu = 0, end_clu = 0, next_clu = 0;
    uint32_t temp1 = 0, temp2 = 0;
    uint8_t res = 0;
...
uint32_t znFAT_WriteData(struct FileInfo *pfi, uint32_t len, uint8_t *pbuf)
{
    uint32_t temp = 0, temp1 = 0, len_temp = len;
    uint32_t Cluster_Size = ((pInit_Args->BytesPerSector) * (pInit_Args->SectorsPerClust));
    ...
    if(len > (0XFFFFFFFF - pfi->File_Size)) return (uint32_t) - 2; //文件大小在写入数据后将发生溢出错误
...
//检查磁盘剩余空间是否够用
    if((pfi->File_CurOffset % Cluster_Size) != 0)
    {
        ...
        if(len > temp) //如果要写入的数据量大于temp, 则说明必然会超出当前簇, 为其扩展空簇
        {
            ...
            if(temp1 > (pInit_Args->Free_nCluster)) return ((uint32_t) - 1); //空间不足
        }
        ...
        if(len <= temp) //要写入的数据小于等于当前扇区剩余数据量
        {
            ...
            if(len == temp) //如果要写入的数据正好占满当前扇区
            {
                ...
                if(0 != pfi->File_CurPos)
                {
                    ...
                    sexb_cur_dev = (uint8_t)(-1); //EXB的当前设备号置为空, 这里取-1认定其为空, 为了与有效设备号0相区分
            ...
            if(!(IS_END_SEC_OF_CLU(pfi->File_CurSec, pfi->File_CurClust))) //如果当前扇区不是当前簇的最后一个扇区
            {
                ...
                if(len_temp <= temp) //如果要写入的数据量小于等于当前簇中的剩余整扇区数据量
                {
                    ...
                    else
                    {
                        ...
                        pfi->File_CurPos = (uint16_t)(len_temp % (pInit_Args->BytesPerSector));
...
uint8_t znFAT_Dump_Data(struct FileInfo *pfi, uint32_t offset)
{
    just_file = pfi;
...
uint32_t znFAT_Modify_Data(struct FileInfo *pfi, uint32_t offset, uint32_t len, uint8_t *app_Buffer)
{
    uint32_t temp_len = 0, temp = 0, nsec = 0, iClu = 0, start_clu = 0, end_clu = 0, next_clu = 0, temp2 = 0, temp1 = 0;
    uint32_t Cluster_Size = ((pInit_Args->BytesPerSector) * (pInit_Args->SectorsPerClust));
    ...
    if(((pfi->File_CurOffset) % Cluster_Size) != 0) //如果当前位置非簇大小整数倍, 即不在簇开始位置
    {
        if(((pfi->File_CurOffset) % (pInit_Args->BytesPerSector)) != 0) //如果当前位置非整扇区大小整数倍, 即不在扇区开始位置
        {
            if(len <= ((pInit_Args->BytesPerSector) - (pfi->File_CurPos))) //如果要修改的数据长度小于等于文件当前扇区剩余数据量
            {
                ...
                else if(len == ((pInit_Args->BytesPerSector) - (pfi->File_CurPos))) //要修改的数据量等于当前扇区剩余数据量
                {
...
uint8_t znFAT_Close_File(struct FileInfo *pfi)
{
...
#ifndef RT_UPDATE_CLUSTER_CHAIN
#ifdef USE_ALONE_CCCB
    pcccb_buf = (uint32_t *)0;
    pcccb_curval = (uint32_t *)0;
    pcccb_counter = (uint8_t *)0;
#endif
#endif
    ...
    Memory_Set((uint8_t *)pfi, (uint32_t)sizeof(struct FileInfo), 0);
