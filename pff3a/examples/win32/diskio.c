...
DWORD WINAPI tmr_thread (LPVOID parms)
{
    ...
    for (;;)
    {
        ...
        {
            Sleep(1);
            if( Stat[drv].h_drive == IHV ||
                Stat[drv].status & STA_NOINIT ||
                WaitForSingleObject(hMutex, 100) != WAIT_OBJECT_0 )
                continue;

            if (!DeviceIoControl(Stat[drv].h_drive, IOCTL_STORAGE_CHECK_VERIFY,
...
BOOL assign_drives (int argc, char *argv[])
{
    ...
    while (pd < 10 && pd < argc - 1)
    {
        ...
        h = CreateFile(str, GENERIC_READ | GENERIC_WRITE, 0,
...
)
{
    ...
    if (Stat[0].sz_sector != 512 ||
        ...
        WaitForSingleObject(hMutex, 3000) != WAIT_OBJECT_0)
        return RES_NOTRDY;

    if (sector >= Stat[0].n_sectors ||
        !count ||
        offset + count > 512 ||
        Stat[0].wip)
    {
    ...
    if (!buff)
    {
        ...
        else
        {
            if (Stat[0].wip)
            {
                ...
                {
                    if (!WriteFile(Stat[0].h_drive, Buffer, 512, &rnc, NULL) ||
                        512 != rnc)
                    {
                        res = RES_ERROR;
                    }
