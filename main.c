...
static void*
_thread_xmodem_receive(void* data)
{
    ...
    while( *pHas_exit == false )
    {
         ...
         */
        nbytes = xmodemReceive((uint8_t*)g_buf_rx, sizeof(g_buf_rx));
        if (nbytes < 0)
...
int main(int argc, char **argv)
{
    ...
    pthread_create(&t1, NULL, _thread_xmodem_receive, &has_exit);
     ...
     */
    nbytes = xmodemTransmit((uint8_t*)g_pBuf_tx, filesize);
    if( nbytes < 0 )
    {
