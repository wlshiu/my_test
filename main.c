...
static void _sim_button_press(button_id_t btn_id, int press_duration_ms)
{
    ...
    for(int i = 0; i < (press_duration_ms >> 2); i++)
    {
        btn_routine(&hBtn);
    }
...
int main()
{
    ...
    btn_key[BUTTON_ID_0].debounce_cnt  = 3;
    btn_key[BUTTON_ID_0].pin_act_lv    = 1;
    btn_key[BUTTON_ID_0].cb_get_pin_state = _get_pin0_state;
    btn_key[BUTTON_ID_0].cb_btn_event     = _btn0_event;

    btn_key[BUTTON_ID_1].debounce_cnt  = 3;
    btn_key[BUTTON_ID_1].pin_act_lv    = 1;
    btn_key[BUTTON_ID_1].cb_get_pin_state = _get_pin1_state;
    btn_key[BUTTON_ID_1].cb_btn_event     = _btn1_event;

    hBtn.button_num = sizeof(btn_key)/sizeof(btn_key[0]);
    hBtn.pBtn_key   = (btn_key_t*)&btn_key;
