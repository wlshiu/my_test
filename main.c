...
crc32_le_generic(uint32_t crc, unsigned char const *p,
 ...
 */
static inline uint32_t
crc32_be_generic(uint32_t crc, unsigned char const *p,
                size_t len, uint32_t polynomial)
{
    int i;
    while (len--) {
...
{
#define SEED    (-1U)
    char        *pData_1 = "test1234";
    char        *pData_2 = "abcdefghi";
    uint32_t    value = SEED;

    value = crc32_le(value, (unsigned char const*)pData_1, strlen(pData_1) + 1);
    printf("crc32: x%x\n", value);
