...
void stb_fft_r2c_exec(stb_fft_real_plan *plan, const stb_real_t *input, cmplx *output)
{
    ...
    for (int c = 1; c <= n / 2; ++c)
    {
        ...
        t.imag = -t.imag;
        stb_real_t r = (plan->twiddles[c - 1].real * (plan->buffer[c].real - t.real)) - (plan->twiddles[c - 1].imag *
                                                                                         (plan->buffer[c].imag - t.imag));
        stb_real_t i = (plan->twiddles[c - 1].real * (plan->buffer[c].imag - t.imag)) + ((plan->buffer[c].real - t.real) *
                                                                                         plan->twiddles[c - 1].imag);
...
void stb_fft_c2r_exec(stb_fft_real_plan *plan, const cmplx *input, stb_real_t *output)
{
    ...
    for (int c = 1; c <= n / 2; ++c)
    {
        cmplx t = input[n - c];
        t.imag = -t.imag;
        stb_real_t r = (plan->twiddles[c - 1].real * (input[c].real - t.real)) + (plan->twiddles[c - 1].imag *
                                                                                  (input[c].imag - t.imag));
        stb_real_t i = (plan->twiddles[c - 1].real * (input[c].imag - t.imag)) - ((input[c].real - t.real) * plan->twiddles[c -
                                                                                                                            1].imag);
