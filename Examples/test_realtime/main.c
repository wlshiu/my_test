...
void BSP_AUDIO_IN_TransferComplete_CallBack(void)
{
    if(kws_frame_len != kws_frame_shift)
    {
        ...
        arm_copy_q7((q7_t *)(kws_audio_buffer) + 2 * (kws_audio_buffer_size - (kws_frame_len - kws_frame_shift)),
                    (q7_t *)kws_audio_buffer, 2 * (kws_frame_len - kws_frame_shift));
    }
    // copy the new recording data
    for (int i = 0; i < kws_audio_block_size; i++)
    {
        kws_audio_buffer[kws_frame_len - kws_frame_shift + i] = audio_buffer_in[i * CHANNELS + DOUBLE_BUFFER * CHANNELS *
                                                                                kws_audio_block_size];
    }
    run_kws();
    return;
}
