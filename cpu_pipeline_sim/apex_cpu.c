...
/* Converts the PC(CONFIG_EXEC_BASE_ADDR series) into array index for code memory
 *
 * Note: You are not supposed to edit this function
 */
static int get_code_memory_index_from_pc(const int pc)
{
    return (pc - CONFIG_EXEC_BASE_ADDR) / 4;
}
...
static void print_stage_content(const char *name, const CPU_Stage *stage)
{
    printf("%-15s: pc(0x%08X) ", name, stage->pc);
...
APEX_cpu_init(const char *filename)
{
    ...
    if (cpu->debug_messages)
    {
        ...
        fprintf(stderr, "APEX_CPU: PC initialized to 0x%X\n", cpu->pc);
...
//  print_rat(cpu);
    ...
    for (int i = 0; i < RENAME_TABLE_SIZE; ++i)
    {
        printf("|   Register: R [%2d]    |   Value: 0x%-8x  |   Status: %-7s    |\n", i,
...
void print_mem(APEX_CPU *cpu)
{
    ...
    for (int i = 0; i < DATA_MEMORY_SIZE; i++)
    {
        ...
        {
            printf("|   Memory [0x%08X]   |   Data Value: 0x%-7x  |\n", i, cpu->data_memory[i]);
            count++;
        }
        if (count == 10) break;
    }
