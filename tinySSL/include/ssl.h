...
typedef struct CERTKEY_INFO
{
    ...
    int            nKeyLengthBits;
    const int8_t    *pCertificate;
    const int8_t    *pPublicKey;
    const int8_t    *pPrivateKey;
} CERTKEY_INFO_T;
...
typedef unsigned int (*SSL_RANDOM)(void);

//The pUserData will be interpreted as a HSSL, SSL Handle.
typedef SSL_RESULT_T (*SSL_CALLBACK)(SSL_PARAMS_T *pParam, HSSL pUserData);
...
SSL_RESULT_T SSL_AddRootCertificate(const int8_t *pCert, int nLen, int nUnixTime);
...
SSL_RESULT_T SSL_Create(HSSL *pHSSL, CERTKEY_INFO_T *pCertKey);

//Destroy an instance of HSSL. The instance should no longer be used.
SSL_RESULT_T SSL_Destroy(HSSL hSSL);

//The filtering function that carries out all SSL operations.
SSL_RESULT_T SSL_Process(SSL_PARAMS_T *pParam, HSSL pUserData);
