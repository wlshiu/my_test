...
unsigned int *to_be;
static unsigned int R0[1024], R1[1024], R2[1024], R3[1024], R4[1024], R5[1024], R6[1024], R7[1024];
static unsigned int R8[1024], R9[1024], R10[1024], R11[1024], R12[1024], R13[1024], R14[1024], R15[1024];
...
void reset_proc()
{
    ...
    for(i = 0; i < 4000; i++)
        MEM[i] = '0';
    PC = 0;
    operand1 = destination = offset = address = flag = opcode = result = immediate = cond = 0;
    operand2 = 0;
    N = C = V = Z = 0;
    //*(to_be) = 0;
}
...
void execute()
{
    ...
    else if(flag == 1)
    {
        if(opcode == 25)
        {
            ...
            {
                result = R0[k];
                printf("EXECUTE: Put in R%d,  R%d's element number %d\n", destination, operand1, k + 1);
            }
            else if(operand1 == 1)
            {
                result = R1[k];
        ...
        else if(opcode == 24)
        {
            ...
            {
                to_be = &R0[k];
                printf("EXECUTE: Put R%d's value in element number %d in R%d\n", operand1, k + 1, destination);
            }
            else if(operand1 == 1)
            {
                to_be = &R1[k];
