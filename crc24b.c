...
/**
 *  CRC24-B  x^24+ x^23+ x^6+ x^5+ x+1
 *  https://github.com/lumpy-zhu/TD-LTE
 */
...
uint32_t crc24b(uint8_t *data, int data_len)
{
    uint32_t crc = 0x000000;

    while (data_len--) {
        uint32_t tbl_idx = ((crc >> 16) ^ *data) & 0xff;
 ...
 * #define CRC24_CHCK  0x800FE3L
 * #define CRC24_SEED  0xFFFFFFL
 *
 * unsigned long
 * compute_crc(
 *     unsigned long   crc,
 ...
 * void main()
 * {
 *     uint32_t     crc24 = compute_crc(CRC24_SEED, data, 54);
 * }
 */
 ...
 * unsigned int             poly24b = 0x80006300;
 *
 * unsigned int
 ...
 * }
 *
 * unsigned int crc24b_2 (unsigned char *inptr, int bitlen)
 * {
 *
 *     int             octetlen, resbit;
 *     unsigned int             crc = 0;
 ...
 *     }while (++c);
 *
 *     crc = crc24b_2(data, sizeof(data) * 8);
 * }
 *
 */
