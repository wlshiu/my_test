...
#define CONFIG_SPIFC_1_SECTOR_SIZE          (4 << 10)
...
static uint8_t      g_fc_sector_cache[CONFIG_SPIFC_1_SECTOR_SIZE] __aligned(4) = {0};
//=============================================================================
    ...
    int                 unit_num)
{
    ...
    do {
        ...
        {
            if( flash_addr & (CONFIG_SPIFC_1_SECTOR_SIZE - 1) )
            {
                rval = SPIFC_ERR_ADDR_NOT_ALIGNMENT;
                break;
            }

            memset(&g_spifc_mem_sim[flash_addr], 0xFF, unit_num * CONFIG_SPIFC_1_SECTOR_SIZE);
        }
    ...
    int         bytes)
{
    ...
    do {
        ...
        offset    = flash_addr & (CONFIG_SPIFC_1_SECTOR_SIZE - 1);
        valid_len = CONFIG_SPIFC_1_SECTOR_SIZE - offset;
        to_pos    = flash_addr & ~(CONFIG_SPIFC_1_SECTOR_SIZE - 1);
        from_pos  = (uintptr_t)pSys_buf;

        // read data to cache
        spifc_read(g_fc_sector_cache, to_pos, CONFIG_SPIFC_1_SECTOR_SIZE);
        ...
        // page program
        for(int i = 0; i < (CONFIG_SPIFC_1_SECTOR_SIZE >> SPIFC_PAGE_SIZE_POW_2); i++)
        {
        ...
        remain -= valid_len;
        while( (remain & ~(CONFIG_SPIFC_1_SECTOR_SIZE - 1)) )
        {
            // erase sector
            spifc_erase(SPIFC_ERASE_SECTOR, to_pos, 1);
            ...
            // update info
            remain -= CONFIG_SPIFC_1_SECTOR_SIZE;
        }
