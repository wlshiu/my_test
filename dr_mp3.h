...
typedef struct
{
    ...
    drmp3_uint16
    pcmFramesToDiscard;    // The number of leading samples to read and discard. These are discarded after mp3FramesToDiscard.
} drmp3_seek_point;
...
typedef struct
{
    ...
    drmp3_uint64
    currentPCMFrame;       // The current PCM frame, globally, based on the output sample rate. Mainly used for seeking.
    drmp3_uint64 streamCursor;          // The current byte the decoder is sitting on in the raw stream.
    drmp3_src src;
    drmp3_seek_point
    *pSeekPoints;      // NULL by default. Set with drmp3_bind_seek_table(). Memory is owned by the client. dr_mp3 will never attempt to free this pointer.
    drmp3_uint32
    seekPointCount;        // The number of items in pSeekPoints. When set to 0 assumes to no seek table. Defaults to zero.
    size_t dataSize;
    size_t dataCapacity;
    drmp3_uint8 *pData;
...
drmp3_bool32 drmp3_seek_forward_by_pcm_frames__brute_force(drmp3 *pMP3, drmp3_uint64 frameOffset)
{
    ...
    drmp3_uint64 maxFramesToReadAndDiscard = (drmp3_uint64)(DRMP3_MAX_PCM_FRAMES_PER_MP3_FRAME * 3 * ((
                                                                                                          float)pMP3->src.config.sampleRateOut / (float)pMP3->src.config.sampleRateIn));
...
drmp3_get_mp3_and_pcm_frame_count(drmp3 *pMP3, drmp3_uint64 *pMP3FrameCount, drmp3_uint64 *pPCMFrameCount)
{
    ...
    float totalPCMFrameCountFractionalPart =
        0; // <-- With resampling there will be a fractional part to each MP3 frame that we need to accumulate.
    for (;;)
    {
