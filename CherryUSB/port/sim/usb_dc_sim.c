...
#define USB_BASE    ((uint32_t)(&g_USBD))
...
#define musb_write_packet       musb_dc_write_packet
#define musb_read_packet        musb_dc_read_packet
//=============================================================================
//                  Macro Definition
//=============================================================================
...
sys_vmsgq_t     g_dc_vmsgq = {0};

uint32_t        g_udev_rxbuf[2048 >> 2] = {0};
uint32_t        g_udev_txbuf[2048 >> 2] = {0};
//=============================================================================
...
//    }
    ...
    pHdr->len = (len < CONFIG_USBHOST_REQUEST_BUFFER_LEN) ? len : CONFIG_USBHOST_REQUEST_BUFFER_LEN;
    ...
    pNode->length = len;
    memcpy((void*)&pNode->raw, buffer, pNode->length);
...
static void musb_read_packet(uint8_t ep_idx, uint8_t *buffer, uint16_t len)
{
    ...
    memcpy(buffer, g_udev_rxbuf, (len < rxbuf_size) ? len : rxbuf_size);
...
//    memset(g_udev_rxbuf, 0x0, sizeof(g_udev_rxbuf));
    ...
    sys_vmsg_node_t     *pNode = 0;
    sys_vmsgq_recv(&g_hc_vmsgq, &pNode);
    if( pNode )
    {
...
int usb_dc_init(void)
{
    ...
    #else
    {
        ...
        memcpy(&pNode->usb_regs, &g_USBD, sizeof(pNode->usb_regs));
...
int usbd_read_packet(uint8_t ep_addr, uint8_t *buffer, uint16_t len)
{
    ...
    if (cnt)
    {
        ...
        HWREGB(USB_BASE + MUSB_IND_RXCSRL_OFFSET) &= ~(USB_RXCSRL1_OVER | USB_RXCSRL1_ERROR | USB_RXCSRL1_STALL |
                                                       USB_RXCSRL1_STALLED);
...
static void dc_handle_ep0(void)
{
...
void USBD_IRQHandler(void)
{
    ...
    memcpy(&g_USBD, &pNode->usb_regs, sizeof(g_USBD));
    memcpy(&g_udev_rxbuf, &pNode->raw, sizeof(pNode->raw));
    free(pNode);
    #endif  /* CONFIG_USB_SIM */
